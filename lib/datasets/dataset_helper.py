    ...
    for list_filename in list_filenames:
        with open(list_filename, 'r') as f:
            ...
            for line in f:
                ...
                img_path = os.path.join(cfg.DATADIR, row[3])
                if 'ucf' in cfg.DATADIR:
                    img_path = img_path.replace('png', 'jpg').encode("ascii")
                
                image_paths[data_key].append(img_path)
...
def get_sequence(center_idx, half_len, sample_rate, num_frames):
    if half_len == 0:
        seq = range(center_idx, center_idx + 1)
    else:

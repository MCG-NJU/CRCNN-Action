...
def _load_and_process_images(
    ...
    for batch in range(batch_size):
        for box_idx in range(len(lbls[batch])):
            ...
            if len(lfb) > 0:
                if cfg.LFB.BANK_TYPE in ['clusters', 'clusters_sample']:
                    out_lfb.append(lfb[batch][box_idx])
                else:
    ...
    out_list = [images, out_labels, out_boxes, out_original_boxes, out_metadata,
                out_lfb]
...
def get_clip_from_source(args):
    ...
    try:
        ...
        if cfg.AVA.SCALE_BOX:
            for i in range(boxes.shape[0]):
                one_box = boxes[i]
                
                if split == 1: # "train"
                    one_box = scale_box(one_box, cfg.AVABOX.TRAIN_BOX_SCALES)
                else:
                    assert cfg.AVABOX.TEST_BOX_SCALES[0] == cfg.AVABOX.TEST_BOX_SCALES[1]
                    one_box = scale_box(one_box, cfg.AVABOX.TEST_BOX_SCALES)

                boxes[i] = one_box
        
        
#         if cfg.AVA.ERASE_BACKGROUND:
#             erase_box = boxes[0].copy()
            
#             if split == 1: # "train"
#                 erase_box = scale_box(erase_box, 
#                                       cfg.AVABOX.TRAIN_BOX_SCALES)
#             else:
#                 erase_box = scale_box(erase_box, 
#                                       cfg.AVABOX.TEST_BOX_SCALES)
            
#             erase_box[[0, 2]] *= width
#             erase_box[[1, 3]] *= height
#             erase_box = imgproc.clip_box_to_image(erase_box, height, width)
#             erase_box = erase_box.astype(np.int)
            
#             for i in range(len(imgs)):
#                 imgs[i][:erase_box[1],:,:]=0
#                 imgs[i][erase_box[3]+1:,:,:]=0

#                 imgs[i][:,:erase_box[0],:]=0
#                 imgs[i][:,erase_box[2]+1:,:]=0

    ...
    for video_idx in range(len(boxes_and_labels)):
        for sec in boxes_and_labels[video_idx].keys():
            ...
            for box_idx in range(len(boxes_and_labels[video_idx][sec])):
                ...
                for c in boxes_and_labels[video_idx][sec][box_idx][1]:
                    each_class_box_indices[c-1].append(count-1)

    logger.info("%d boxes used." % count)

    return box_indices, each_class_box_indices
...
class AvaboxDataset():
    ...
    def __init__(self, split, lfb_infer_only, shift=None, lfb=None, suffix=''):
        ...
        self._return_scene_labels = False
        if cfg.AVABOX.USE_SCENE_LOSS:
            self.blobnames.append('scene_labels'+suffix)
            self._return_scene_labels = True
            
            
        self._return_scene_feat = False
        if cfg.AVABOX.ENABLE_SCENE_FEAT_BANK:
            self.blobnames.append('scene_feat'+suffix)
            self._return_scene_feat = True
            
            if split == 'train':
                sfb_path = os.path.join(cfg.AVABOX.SCENE_FEAT_BANK_DIR,
                                        'train_lfb.pkl')
            else:
                sfb_path = os.path.join(cfg.AVABOX.SCENE_FEAT_BANK_DIR,
                                        'val_lfb.pkl')
                
            self._sfb = pickle.load(open(sfb_path,'rb'))
    ...
    def get_minibatch_info(self, indices):
        ...
        if self._return_scene_labels:
            scene_labels = []
            
        if self._return_scene_feat:
            scene_feat = []
        ...
        for idx in indices:
            ...
            if self._split == 'train':
#                 rand_idx = np.random.choice(range(len(self._box_indices)))
                if cfg.AVABOX.REWEIGHTED_SAMPLE:
                    class_idx = np.random.randint(0, 80)
                    rand_idx = np.random.randint(0, len(self._each_class_box_indices[class_idx]))
                    rand_idx = self._each_class_box_indices[class_idx][rand_idx]
                else:
                    rand_idx = np.random.randint(0, len(self._box_indices))
    
                video_idx, sec, center_idx, box_idx = self._box_indices[rand_idx]
            else:
                video_idx, sec, center_idx, box_idx = self._box_indices[idx]
            ...
            if cfg.AVABOX.CONCAT_GLOBAL_FEAT and cfg.AVABOX.USE_LONG_TERM_FEAT:
                
                long_term_seq_len = self._video_length * cfg.AVABOX.LONG_SAMPLE_RATE
                
                long_term_half_len = long_term_seq_len // 2
                
                long_term_seq = dataset_helper.get_sequence(
                    center_idx, long_term_half_len, cfg.AVABOX.LONG_SAMPLE_RATE,
                    num_frames=len(self._image_paths[video_idx]))
                
                long_term_img_list = [self._image_paths[video_idx][frame] for frame in long_term_seq]
                
                image_paths.append([img_list, long_term_img_list])
            else:
            ...
            if self._return_scene_labels:
                one_frame_labels=set()
                
                for i in range(len(self._boxes_and_labels[video_idx][sec])):
                    
                    if cfg.AVABOX.ERASE_SCENE_LABELS and i == box_idx:
                        continue
                    _, box_labels = self._boxes_and_labels[video_idx][sec][i]
                    
                    for l in box_labels:
                        one_frame_labels.add(l)
                
                scene_labels.append(list(one_frame_labels))
                
            if self._return_scene_feat:
                scene_feat.append(self._sfb[video_idx][sec][0].reshape(2048,1,1,1))
            
            
            #logger.info("labels: %d, lfb:%d" %  (len(labels[-1]), len(self._lfb[video_idx][sec])) )
            #assert (len(self._lfb[video_idx][sec]) >= len(labels[-1]))
        ...
        return_list = [image_paths, labels, split_list, spatial_shift_positions,
                    video_indices, secs, lfb]

        if self._return_scene_labels:
            return_list.append(scene_labels)
        
        if self._return_scene_feat:
            return_list.append(scene_feat)
    ...
    def _get_data(self):
        ...
        self._box_indices, self._each_class_box_indices = get_box_indices(self._boxes_and_labels)
...
def sample_lfb(in_video_lfb, sec, is_train):
    """
    Sample LFB.
...
#                     video_lfb[j * K + k] = in_video_lfb[si][rand_idx]
                    ...
                    video_lfb[j * K + k] = in_video_lfb[si][rand_idx][:cfg.LFB.LFB_DIM]
    else:

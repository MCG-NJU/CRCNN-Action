    ...
    def get_minibatch_info(self, indices):
        ...
        for idx in indices:
            ...
            if self._split == 'train':
                ...
                rand_idx = np.random.randint(0, len(self._keyframe_indices))
            ...
            for _, box_labels in self._boxes_and_labels[video_idx][sec]:
                for l in box_labels:
                    one_frame_labels.add(l)
                        
            one_frame_labels = list(one_frame_labels)

            clip_label_list = [ [np.array([0.0, 0.0, 1.0, 1.0]), one_frame_labels] ]

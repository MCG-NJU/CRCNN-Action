...
from utils.ava_evaluation import object_detection_evaluation
...
def evaluate_ava(preds, original_boxes, metadata,
                 excluded_keys, class_whitelist, categories, groundtruth=None,
                 video_idx_to_name=None, name='latest'):
    """Run AVA evaluation given numpy arrays."""

    eval_start = time.time()

    detections = get_ava_eval_data(
        preds, original_boxes, metadata, class_whitelist,
    ...
    filename = 'detections_%s_%.4f.csv' % (name,
                                        results['PascalBoxes_Precision/mAP@0.5IOU'])
    filename = os.path.join(cfg.CHECKPOINT.DIR, filename)
    write_results(detections, filename)
...
def get_ava_eval_data(scores, boxes, metadata, class_whitelist, verbose=False,

...
class MetricsCalculator():
    ...
    def __init__(self, model, split, video_idx_to_name, total_num_boxes):
        ...
        if cfg.DATASET in ['ava', 'avabox']:
            ...
            if cfg.JHMDB.ENABLED:
                excluded_filename = os.path.join(cfg.JHMDB.ANNOTATION_DIR,
                                     "excluded_timestamps.csv")
                labelmap_filename = os.path.join(cfg.JHMDB.ANNOTATION_DIR,
                                    "action_list.pbtxt")
                gt_filename = os.path.join(cfg.JHMDB.ANNOTATION_DIR, 
                                      cfg.JHMDB.EVAL_GT_FILENAME)
                
            else:
                excluded_filename = os.path.join(cfg.AVA.ANNOTATION_DIR,
                                     "ava_val_excluded_timestamps_v2.1.csv")
                labelmap_filename = os.path.join(cfg.AVA.ANNOTATION_DIR,
                                    "ava_action_list_v2.1_for_activitynet_2018.pbtxt")
                gt_filename = os.path.join(cfg.AVA.ANNOTATION_DIR, "ava_val_v2.1.csv")
                
                
            self.excluded_keys = read_exclusions(excluded_filename)

            self.categories, self.class_whitelist = read_labelmap(labelmap_filename)
    ...
    def finalize_metrics(self, is_train=False, name='latest'):
        ...
        if cfg.MODEL.MULTI_LABEL:
            if is_train:
                ...
                elif cfg.DATASET in ['ava', 'avabox']:
    ...
    def calculate_and_log_all_metrics_train(
            self, curr_iter, timer, suffix='', tb_writer=None):
        ...
        if (curr_iter + 1) % cfg.LOG_PERIOD == 0:
            ...
            if tb_writer is not None:
                tb_writer.add_scalar('Train/Loss', cur_loss, 
                                    curr_iter+1)

                tb_writer.add_scalar('Train/LR', self.lr, 
                                    curr_iter+1)
...
def get_multi_gpu_outputs(split, suffix):
    ...
    for idx in range(cfg.ROOT_GPU_ID, cfg.ROOT_GPU_ID + cfg.NUM_GPUS):
        ...
        if cfg.TEST.SHOW_FEAT:
            feat = workspace.FetchBlob('gpu_{}/msnl_msnl_branch0_layer0_affinity_prob'.format(idx))
            print(feat.shape)
            print(feat)
        ...
        if cfg.JHMDB.ENABLED and cfg.JHMDB.USE_SOFTMAX_LOSS:
            softmax = scipy.special.softmax(softmax, axis=1)

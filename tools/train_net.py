...
def create_wrapper(is_train, lfb=None):
    ...
    meter = metrics.MetricsCalculator(
        ...
        total_num_boxes=(model.input_db._num_boxes_used if cfg.DATASET in ['ava', 'avabox']
                         else None)
    )
...
def train(opts):
    ...
    for curr_iter in range(start_model_iter, cfg.SOLVER.MAX_ITER):
        ...
        train_meter.calculate_and_log_all_metrics_train(
            curr_iter, train_timer, suffix='_train', tb_writer=tb_writer)
        ...
        if (curr_iter + 1) % cfg.TRAIN.EVAL_PERIOD == 0:
            ...
            tb_writer.add_scalar('Test/mini_MAP', test_meter.full_map, curr_iter+1)
